...
`.github/workflows/scan.yaml` runs `deno task scan` daily and commits the
...
half an hour after it, and needs no secrets at all. Both are scheduled in the
small hours Pacific; the gap between them is there because both push to trunk,
and a simultaneous second push would be rejected. The Dropout run takes a
deliberately small bite each day, and a manual dispatch can pass a larger
`budget` to bulk backfill. Each run also chips away at the back catalogue of
unfetched release dates until it converges.
