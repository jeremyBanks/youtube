...
export default class Session extends EventEmitterLike {
  ...
  static async create(options: SessionOptions = {}) {
    const { context, api_key, api_version, account_index } =
  ...
  static async #retrieveSessionData(
    ...
    if (options.visitor_data) {
      const decoded_visitor_data = Proto.decodeVisitorData(
        options.visitor_data
      );
  ...
  static #generateSessionData(options: {
    ...
    const context: Context = {
      client: {
        ...
        screenPixelDensity: 1,
        screenWidthPoints: 1920,
        visitorData: Proto.encodeVisitorData(
          visitor_id,
    ...
    return {
      context,
      api_key: Constants.CLIENTS.WEB.API_KEY,
      api_version: Constants.CLIENTS.WEB.API_VERSION,
    };
  }
